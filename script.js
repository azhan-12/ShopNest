...
dropdowns.forEach(drop => {
  const toggleLink = drop.querySelector('a'); // Only the top-level dropdown link

  toggleLink.addEventListener('click', (e) => {
    if (window.innerWidth <= 768) {
      e.preventDefault();  // Prevent navigation on mobile toggle click
      drop.classList.toggle('active'); // Toggle dropdown open/close
    }
  });
});
